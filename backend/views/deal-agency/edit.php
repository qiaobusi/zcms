    ...
    <section class="content">
    	<div class="row">
            ...
            <div class="box-header with-border">
              <h3 class="box-title">担保机构</h3>
            </div>
            <form class="form-horizontal" id="form" action="<?php echo Url::to(['deal-agency/save']); ?>">
            	<input name="_csrf-backend" type="hidden" id="_csrf" value="<?php echo Yii::$app->request->csrfToken; ?>">
            	<input name="id" type="hidden" id="id" value="<?php echo $id; ?>" >
              <div class="box-body">
                <div class="form-group">
                  <label for="name" class="col-sm-2 control-label">担保机构</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" id="name" name="DealAgency[name]" placeholder="担保机构" value="<?php echo $dealAgency->name; ?>">
  		...
  		$(function(){
					...
					$('#saveForm').click(function(){
							...
							$.ajax({
									...
									data: {
											'id': id,
											'DealAgency[name]': name,
											'DealAgency[is_effect]': is_effect,
											'_csrf-backend': $('#_csrf').val()
									},
									dataType: 'JSON',
									success: function(data) {
											if (data.status == 1) {
													window.location.href = '<?php echo Url::to(['deal-agency/index']); ?>';
											} else {
